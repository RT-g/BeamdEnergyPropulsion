...
%% Physicsal constatnts
    ...
    e_vib_N2 = 0.288;        % ’‚‘f•ªŽq‚ÌU“®“Á«ƒGƒlƒ‹ƒM[ [eV](2016_Electron-Vibrational energy exchange), 3380 K(ŽÀÝ‹C‘Ì—¬‚ê‚Ì—ÍŠw)
    e_vib_O2 = 0.196;        % Ž_‘f•ªŽq‚ÌU“®“Á«ƒGƒlƒ‹ƒM[ [eV](2016_Electron-vibration relaxation in oxygen plasmas), 2230 K(ŽÀÝ‹C‘Ì—¬‚ê‚Ì—ÍŠw)

    
%% “d—£C“dŽq—ã‹N‚ÉŠÖ‚·‚é’è” / Contants of ionziation and excitation
% ’‚‘f•ªŽq‚ÉŠÖ‚·‚é’è” / Constatnts of nitrogen molecules
    epsilon_ion_N2 = 15.58;    % “d—£ƒGƒlƒ‹ƒM[ / Ionization energy [eV]
    epsilon_ex_N2(1) = 6.17;   % A(v=0-4), “dŽq—ã‹N€ˆÊ‚ÌƒGƒlƒ‹ƒM[ [eV]
...
% Ž_‘f•ªŽq‚ÉŠÖ‚·‚é’è”
    epsilon_ion_O2 = 12.07;   % “d—£ƒGƒlƒ‹ƒM[ / Ionization energy [eV]
    epsilon_ex_O2(1) = 0;     % Šî’ê€ˆÊ
...
% Absoprtion layer
    length_abs = lambda;                       % ‹zŽû’·
    num = round(length_abs/dx);       % ‹zŽû—Ìˆæ‚ÌƒOƒŠƒbƒh”
    if num == 0
        num = 1;
    end
...
% Absorption layer
    ...
    q_abs = u_m/omega;
    k_0 = omega/c;
    l_abs = 1/(k_0/2^0.5*(r_abs-1+((1-r_abs^2)+(q_abs*r_abs)^2)^0.5)^0.5);   % Absorption depth (170 GHz‚Ìê‡0.2mm’ö“x)
...
%--------------------------- Pre-allocation -----------------------------%
    ...
    rho = zeros(1,N);
    u = zeros(1,N);
    u2 = zeros(1,N);                    % “d—£”g–ÊŒÅ’èŒn‚É‘Î‚·‚é2ŽŸ‘¤‚Ì—¬‘¬
    P = zeros(1,N);
    T_gas = zeros(1,N);
    T_vib = zeros(1,N);
    H = zeros(1,N);
    ...
    r_thruster = zeros(1,N);
    A_thruster = zeros(1,N);
    V_thruster = zeros(1,N);
    Angle =zeros(1,N);
    ...
    Thrust = zeros(1,2);
    Impulse_add = zeros(1,2);
    
    Pressure = zeros(1,N);
    P_closed = zeros(1,2);
    P_open = zeros(1,2);
    Velocity = zeros(1,N);
    Temperature_tr_rot = zeros(1,N);
    Temperature_vib = zeros(1,N);
    Density = zeros(1,N);
    Mach = zeros(1,N);
...
%--------------------------- Nozzle configaration ------------------------------%
    % Thruster configuration
    eta = 0.486225;                            % Heat absorption efficiency
    phi = 1;                              % Plasma space occupancy
    omega_0 = 20.4e-3;                         % Beam radius [mm]
    r = 30e-3;                                 % Thruster's diameter [mm]
    
    x_conv = 100e-3;                           % Converge starting point [m] 
    Angle_conv = 0*pi/180;                   % Convergence angle [rad]    => 0 when using a normal tube
    x_div = 120e-3;                            % Diverge starting point [m]
    Angle_div = 0*pi/180;                     % Divergence angle [rad]     => 0 when using a normal tube
    
    % Thruster's tube radius
    for i = 1:x_conv/dx
        r_thruster(i) = r;
        Angle(i) = 0;
    end
    for i = x_conv/dx:x_div/dx
        r_thruster(i) = r + tan(Angle_conv)*(i*dx-x_conv);
        Angle(i) = Angle_conv;
    end
    for i = round(x_div/dx):N
        r_thruster(i) = r + tan(Angle_conv)*(x_div-x_conv) + tan(Angle_div)*(i*dx-x_div);
        Angle(i) = Angle_div;
    end
    
    % Thruster's tube area
        A_thruster = pi*r_thruster.^2;
        
%% Calculation of Area and Volume at each cell
    for i = 1:N-1
        A_half(i) = ((A_thruster(i).^0.5 + A_thruster(i+1).^0.5)/2).^2;
    end
        A_half(N) = A_half(N-1);
    
    for i = 2:N
        if A_half(i) == A_half(i-1)
            V_thruster = pi*r_thruster.^2;
        else
            V_thruster(i) = dx/3*(A_half(i).^1.5-A_half(i-1).^1.5)/(A_half(i).^0.5-A_half(i-1).^0.5);
        end
    end
        V_thruster(1) = V_thruster(2);
...
for step = 0:N_t
   ...
   %% Time step update
    ...
    if mod(t,1e-5) < dt
        t*1e+3    % 10 us‚²‚Æ‚ÉŽžŠÔ‚ð•\Ž¦[ms]
        step/N_t  % ŒvŽZ‚Ìi‚Ý‹ï‡‚ð•\Ž¦
    end
    
   %% The placement of shock wave => MSC‚Ì‚Æ‚«‚±‚Ì•û–@‚Í‚¤‚Ü‚­‚¢‚©‚È‚¢‚Ì‚Å‰ü‘P‚ª•K—v
   %if mod(t,1e-6) < dt
   ...
   %% Energy source term => ‹zŽû’·‚ð”g’·’ö“x‚Å—^‚¦‚é
   
   % Heat input by ionization front propagation
    % Source term
        for i = 1:N
            if t*U_ion < length
                if t*U_ion-length_abs <= i*dx && i*dx <= t*U_ion
                    w(i) = eta * 2 *(omega_0/(2*r_thruster(i)))^2 * S_peak*1e+9 / (num*dx);
                else
                    w(i) = 0;
                end
            else
    ...
    % Vibrational-translational relaxation energy
        % k_2 ‚ÌŒvŽZ
            %k_VT_1 = 7.8e-18.*T_gas.*exp(-218./(T_gas.^(1/3))+690./T_gas)./(1-exp(-1*q*e_vib_N2./(kB.*T_gas)));
        ...
        % ŠÉ˜aŽžŠÔ‚ÌŒvŽZ
            %tc = 1./(1-exp(-1*q*e_vib_N2./(kB.*T_gas))).*(k_VT_1.*n_air+k_VT_2.*n_N);
            tc = 1./(7e-10.*exp(-141.*T_gas.^(-1/3)).*n_air*1e-6);
            
        % ƒGƒlƒ‹ƒM[ƒ\[ƒX€‚ÌŒvŽZ
            Q_VT = n_air.*(E_v_1-E_v_3)./tc; % [J/m^3]
  
            
    % ‰^“®—Ê—A‘—ƒGƒlƒ‹ƒM[ / Elastic collision energy transfer
            %n_e_d = (n_air*4.82e+21.*T_vib.^1.5.*exp(-11600*epsilon_ion_N2./T_vib)).^0.5;
            Q_ela = 3*m_e/m_air*kB.*5.3e+9.*P*760/101300.*(T_vib-T_gas).*1e+21*P_ambient/101300;            % ‘S—Ìˆæ‚Å1e+21‚É‚µ‚Ä‚¢‚é‚Ì‚ÅCŠÉ˜a‚ª‘¬‚¢‰Â”\«‚ª‚ ‚éD
   ...
   %% Solve Euler equation
    % Time evolution
       ...
       % Time evolution
        Q(1,2:N-1) = Q(1,2:N-1) - dt/dx* (E_half(1,2:N-1) - E_half(1,1:N-2)) - E(1,2:N-1).*(log(A_thruster(2:N-1))-log(A_thruster(1:N-2)))*dt/dx;
        Q(2,2:N-1) = Q(2,2:N-1) - dt/dx* (E_half(2,2:N-1) - E_half(2,1:N-2)) - (E(2,2:N-1)-P(2:N-1)).*(log(A_thruster(2:N-1))-log(A_thruster(1:N-2)))*dt/dx;
        Q(3,2:N-1) = Q(3,2:N-1) - dt/dx* (E_half(3,2:N-1) - E_half(3,1:N-2)) - E(3,2:N-1).*(log(A_thruster(2:N-1))-log(A_thruster(1:N-2)))*dt/dx + Q_gas(2:N-1) * dt;                % •ÀiE‰ñ“]ƒGƒlƒ‹ƒM[•Û‘¶‘¥
        Q(4,2:N-1) = Q(4,2:N-1) - dt/dx* (E_half(4,2:N-1) - E_half(4,1:N-2)) - E(4,2:N-1).*(log(A_thruster(2:N-1))-log(A_thruster(1:N-2)))*dt/dx + Q_electron(2:N-1) * dt;           % U“®ƒGƒlƒ‹ƒM[•Û‘¶‘¥, ‹óŠÔè—L—¦‚ðl—¶‚µ‚Ä“ü”M
    ...
    %% Thrust impulse calculation
     if mod(t,1e-6) < dt
         Y = [Y; t*1e+3];
         Thrust = [Thrust; t*1e+3 (P(1)-P_ambient)*pi*r_thruster(1)^2+sum((P-P_ambient)*2*pi.*r_thruster*dx.*sin(Angle))];
         Impulse = Impulse + 1e-6*((P(1)-P_ambient)*pi*r_thruster(1)^2+sum((P-P_ambient)*2*pi.*r_thruster*dx.*sin(Angle)));
         Impulse_add = [Impulse_add; t*1e+3 Impulse];
         Pressure = [Pressure; P];
         P_closed = [P_closed; t*1e+3 P(1)];
         P_open = [P_open; t*1e+3 P(N-1)];
         Velocity = [Velocity; u_av];
         Temperature_tr_rot = [Temperature_tr_rot; T_gas];
         Temperature_vib = [Temperature_vib; T_vib];
         Density = [Density; rho];
         Mach = [Mach; u_av./c_av];
     end
   ...
   % Output for Graphix
    ...
    dP = zeros(1,2);
    [rows columns] = size(P_closed);
    for i = 1:rows-1
        dP = [dP; P_closed(i,1) (P_closed(i+1,2)-P_closed(i,2))/(P_closed(i+1,1)-P_closed(i,1))];
    end
    
    d2P = zeros(1,2);
    [rows columns] = size(dP);
    for i = 1:rows-1
        d2P = [d2P; dP(i,1) (dP(i+1,2)-dP(i,2))/(dP(i+1,1)-dP(i,1))];
    end