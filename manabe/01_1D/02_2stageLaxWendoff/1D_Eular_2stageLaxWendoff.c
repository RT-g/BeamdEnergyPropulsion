...
void setNEXT(int t){
    ...
    if(t%2 == 0){
        ...
        for(i=1;i<Nx-1;i++){
            ...
            u_next[i] = (rho_last[i]*u_last[i] - 2*dt*(rho[i+1]*u[i+1]*u[i+1]+p[i+1]-rho[i-1]*u[i-1]*u[i-1]-p[i-1])/2.0/dx) / (rho_next[i]);
            if(((Nx) - (int)(0.01 + lambda/dx))/2<=i&&i<((Nx) + (int)(0.01 + lambda/dx))/2){
                E_next[i] = E_last[i] - 2 * dt * ( ( E[i+1] + p[i+1] ) * u[i+1] - ( E[i-1] + p[i-1] ) * u[i-1] ) / 2.0 / dx + Q_in * 2 * dt;
            }else{
                E_next[i] = E_last[i] - 2 * dt * ( ( E[i+1] + p[i+1] ) * u[i+1] - ( E[i-1] + p[i-1] ) * u[i-1] ) / 2.0 / dx;
            }
            p_next[i] = (Gamma-1.0)*(E_next[i]-(rho_next[i]*u_next[i]*u_next[i])*0.5);
        }
    }
...
int main(){
    ...
    fp1 = fopen("1D_Eular_2stageLaxWendoff_p.csv","w");
    fp2 = fopen("1D_Eular_2stageLaxWendoff_u.csv","w");
    fp3 = fopen("1D_Eular_2stageLaxWendoff_T.csv","w");
    fp4 = fopen("1D_Eular_2stageLaxWendoff_rho.csv","w");
    int i,j;
    setIC();
    for(i=0;i<=Nt;i++){
        setNEXT(i);
